...
채팅 응답은 기본적으로 더미 `echo` 모드로 동작합니다.  
필요하면 Google Gemini를 연결할 수 있고, 그때는 비용 효율이 높은 `gemini-2.5-flash-lite` 모델을 사용합니다.  
...
API 파일들이 공통으로 쓰는 응답/직렬화 헬퍼를 모아 둔 파일입니다.  
덕분에 각 API 파일은 "무슨 흐름으로 처리하는지"에 더 집중할 수 있습니다.
...
user 메시지를 저장하고, Gemini를 호출해 assistant 응답을 만든 뒤 그 응답도 DB에 저장합니다.  
테스트에서는 외부 API 없이 돌릴 수 있도록 `assistant_mode="echo"`도 지원합니다.

### `app/llm.py`

실제 Gemini 호출을 담당하는 아주 단순한 파일입니다.  
`generate_assistant_reply(...)` 함수 하나를 직접 호출하면 되도록 최대한 평평하게 두었습니다.

### `app/db/connection.py`

DB 연결만 담당하는 파일입니다.  
환경변수를 직접 읽어서 `DATABASE_URL`을 만들고, SQLAlchemy `engine`을 생성합니다.  
간단한 connection/transaction 컨텍스트 매니저도 여기서 제공합니다.

### `app/db/queries.py`

room/message 조회에 공통으로 쓰는 작은 DB 헬퍼들을 모아 둔 파일입니다.  
API끼리 서로 내부 함수를 참조하지 않도록 중간 정리 역할을 합니다.
...
기존 room 목록을 보고 선택하거나 새 room을 만든 뒤, 입력을 반복해서 `/api/v1/chat`을 호출하는 chat loop를 제공합니다.

### `app/frontend.html`

브라우저에서 가장 단순하게 채팅을 해볼 수 있는 화면입니다.  
프론트엔드 프레임워크 없이 HTML, CSS, JavaScript만 사용하고, 같은 서버의 API를 직접 호출합니다.  
스크립트도 room 목록, room 선택, 메시지 전송 흐름이 읽히도록 작은 함수들로 나눠 두었습니다.

### `MESSAGE_LOG_MANAGEMENT.md`

현재 프로젝트의 메시지 저장 규칙과 로그 구조를 따로 설명한 문서입니다.  
`message_order`, room별 히스토리, chat API 저장 방식을 따로 보고 싶을 때 참고하면 됩니다.
...
- `GEMINI_API_KEY`: Gemini를 실제로 연결할 때만 필요한 Google AI Studio API 키
- `GEMINI_MODEL`: 사용할 Gemini 모델 이름
- `CHAT_ASSISTANT_MODE`: 기본 assistant 모드. 현재 기본값은 `echo`, 필요하면 `gemini`로 바꿉니다
...
이 화면은 페이지가 처음 열릴 때 room을 하나 자동으로 만들고, 이후에는 같은 room을 브라우저 저장소에 기억해 둡니다.  
이미 room이 있으면 화면 왼쪽의 room 목록에서 이전 대화를 골라 이어갈 수 있습니다.  
`새 대화` 버튼을 누르면 새 room을 만들어서 다시 시작합니다.
...
채팅 1턴을 처리합니다.  
user 메시지를 저장한 뒤, Gemini assistant 응답을 만들고 그 응답도 저장합니다.
...
기본값은 `echo` 모드입니다.  
실제 Gemini 응답을 쓰려면 `.env`에서 `CHAT_ASSISTANT_MODE=gemini` 로 바꾸고 `GEMINI_API_KEY`를 넣어야 합니다.
...
- `/rooms`: 현재 room 목록 보기
- `/switch`: 다른 room 선택 또는 새 room 만들기
- `/exit`: 종료
